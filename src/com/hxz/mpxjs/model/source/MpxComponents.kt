...
import com.hxz.mpxjs.index.getMpxIndexData
...
class MpxComponents {
  companion object {
    ...
    fun mpxMixinDescriptorFinder(implicitElement: JSImplicitElement): MpxSourceEntityDescriptor? {
      getMpxIndexData(implicitElement)?.descriptorRef
        ?.takeIf { it.isNotBlank() }
        ?.let { resolveReferenceToMpxComponent(implicitElement, it) }
        ?.let { return it }

      val mixinObj = (implicitElement.parent as? JSProperty)?.parent as? JSObjectLiteralExpression
      if (mixinObj != null) return MpxSourceEntityDescriptor(mixinObj)
      ...
      if (call != null) {
        return JSStubBasedPsiTreeUtil.findDescendants(call, JSStubElementTypes.OBJECT_LITERAL_EXPRESSION)
          ...
          ?.let { MpxSourceEntityDescriptor(it) }
      }
      return null
    }

    fun resolveReferenceToMpxComponent(element: PsiElement, reference: String): MpxSourceEntityDescriptor? {
      val scope = createLocalResolveScope(element)

      return JSStubBasedPsiTreeUtil.resolveLocally(reference, scope)
               ?.let<PsiElement, MpxSourceEntityDescriptor?> { getMpxComponentFromResolve(listOf(it)) }
               ?.let { return it }
             ?: getMpxComponentFromResolve(ES6QualifiedNameResolver(scope).resolveQualifiedName(reference))
    }
    ...
    private fun getMpxComponentFromResolve(result: Collection<PsiElement>): MpxSourceEntityDescriptor? {
      return result.mapNotNull(::getComponentDescriptor).firstOrNull()
    }

    fun getClassComponentDescriptor(clazz: JSClass): MpxSourceEntityDescriptor =
      MpxSourceEntityDescriptor(
    ...
    fun getComponentDescriptor(element: PsiElement?): MpxSourceEntityDescriptor? =
      when (val resolved = resolveElementTo(element, JSObjectLiteralExpression::class, JSCallExpression::class, JSClass::class)) {
        // {...}
        is JSObjectLiteralExpression -> MpxSourceEntityDescriptor(resolved)

        // Mpx.extend({...})
        // defineComponent({...})
        is JSCallExpression ->
          if (isDefineComponentOrMpxExtendCall(resolved)) {
        ...
        is JSClass ->
          MpxSourceEntityDescriptor(getComponentDecorator(resolved)?.let { getDescriptorFromDecorator(it) },
                                    resolved)
        else -> null
      }
    ...
    @StubUnsafe
    fun isDefineComponentOrMpxExtendCall(callExpression: JSCallExpression): Boolean =
...
open class MpxSourceEntityDescriptor(val initializer: JSElement? /* JSObjectLiteralExpression | JSFile */ = null,
  ...
  fun <T> getCachedValue(provider: (descriptor: MpxSourceEntityDescriptor) -> CachedValueProvider.Result<T>): T {
    ...
    return when {
      clazz != null -> {
        ...
        CachedValuesManager.getCachedValue(theClass, providerKey) {
          val descriptor = MpxComponents.getClassComponentDescriptor(theClass)
          provider(descriptor)
        }
      }
      initializer != null -> {
        ...
        CachedValuesManager.getCachedValue(theInitializer, providerKey) {
          provider(MpxSourceEntityDescriptor(theInitializer))
        }
      }
      else -> {
        val theSource = source
        CachedValuesManager.getCachedValue(theSource, providerKey) {
          provider(MpxSourceEntityDescriptor(source = theSource))
        }
      }
    }
  }
