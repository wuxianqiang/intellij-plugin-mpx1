...
class MpxComponentsCalculation {
  companion object {
    fun calculateScopeComponents(scope: GlobalSearchScope, globalize: Boolean): ComponentsData {
      val allValues = getForAllKeys(scope, MpxComponentsIndex.KEY)
      ...
      for (value in allValues) {
        val indexData = getMpxIndexData(value) ?: continue
        val name = indexData.originalName
        val isGlobal = indexData.isGlobal || globalize
        if (isGlobal && name.endsWith(GLOBAL_BINDING_MARK)) {
          // we come here when in Mpx.component() first argument is a reference,
          // i.e. we do not know the name of global component at the indexing moment
          // two cases are possible: single registration Mpx.component(SomeComp.name, SomeComp)
          // or iteration-registration, when components references are gathered into some object as properties,
          // and further iterated calling Mpx.component()
          // this is especially hard to distinguish them for cases like MpxResolveTest.testResolveWithExplicitForInComponentsBinding
    ...
    private fun findObjectLiteralOfGlobalRegistration(element: JSImplicitElement):
      ...
      val indexData = getMpxIndexData(element)
    ...
    // resolves name of 'singular' registration of Mpx.component(ref (SomeComp.name or ref = 'literalName'), ref (SomeComp))
    private fun resolveGlobalComponentName(element: JSImplicitElement,
      ...
      if (parts.size == 2) {
        // allow only Mpx.component(SomeComp.name, SomeComp) form
        if (parts[0] != indexData.descriptorRef) return null
        // for functional components style, where there is no descriptor - heuristics (mpxtify)
        ...
        when (element) {
          ...
          is JSProperty -> {
            ...
            if (propName != null) {
              ...
              // name used in call Mpx.component() overrides what was set in descriptor itself
    ...
    private fun selectComponentDefinition(list: List<Pair<PsiElement, Boolean>>): Pair<PsiElement, Boolean> {
      ...
      for (componentData in list) {
        val isMpx = MpxFileType.INSTANCE == componentData.first.containingFile.fileType
        if (componentData.second) {
          if (isMpx) return componentData
          selected = componentData
        }
        else if (selected == null && isMpx) selected = componentData
      }
