...
class DataFunctionInspection : LocalInspectionTool() {
    ...
    return object : JSElementVisitor() {
      override fun visitJSProperty(node: JSProperty?) {
        ...
        if (isComponent(node)) {
          ...
          holder.registerProblem(node.nameIdentifier!!, MpxBundle.message("mpx.inspection.message.data.property.should.be.function"),
                                 *quickFixes)
        }
      }
  ...
  fun isComponent(property: JSProperty): Boolean {
    return property.parent is JSObjectLiteralExpression && property.parent.parent is JSExportAssignment &&
           property.containingFile.fileType == MpxFileType.INSTANCE
  }
}

class WrapWithFunctionFix(psiElement: PsiElement) : LocalQuickFixOnPsiElement(psiElement) {
  override fun getText(): String = MpxBundle.message("mpx.quickfix.wrap.with.function.text")
  ...
  override fun getFamilyName(): String = MpxBundle.message("mpx.quickfix.wrap.with.function.family")
