...
import com.hxz.mpxjs.codeInsight.tags.MpxInsertHandler.Companion.reformatElement
import java.util.*

class MpxComponentInplaceIntroducer(elementToRename: XmlTag,
                                    editor: Editor,
                                    private val data: MpxExtractComponentDataBuilder,
  ...
  @Nls
  private val commandName = MpxBundle.message("mpx.template.intention.extract.component")
  ...
  companion object {
    const val GROUP_ID: String = "MpxExtractComponent"
  }
  ...
  private fun askAndRestartRename(@Nls error: String, commandProcessor: CommandProcessor, tag: XmlTag) {
    askConfirmation(error,
                    onYes = {
                      hijackCommand()
                      commandProcessor.executeCommand(myProject, {
                        MpxComponentInplaceIntroducer(tag, myEditor, data, oldText,
  ...
  private fun askConfirmation(@Nls title: String, onYes: () -> Unit, onNo: () -> Unit) {
    val yesText = MpxBundle.message("mpx.template.intention.extract.component.continue")
