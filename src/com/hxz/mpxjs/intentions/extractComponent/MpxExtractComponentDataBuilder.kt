...
import com.hxz.mpxjs.index.MpxFileVisitor
...
class MpxExtractComponentDataBuilder(private val list: List<XmlTag>) {
  ...
  private fun processMpxComponent(ref: WebSymbolTagNameReference) {
...
export default {
  ...
  private fun psiOperationOnText(text: String, operation: (PsiFile) -> Unit): String {
    val dummyFile = PsiFileFactory.getInstance(containingFile.project).createFileFromText("a.mpx", MpxFileType.INSTANCE, text)
    operation(dummyFile)
  ...
  fun modifyCurrentComponent(newComponentName: String, currentFile: PsiFile, newPsiFile: PsiFile, editor: Editor) {
    MpxInsertHandler.InsertHandlerWorker().insertComponentImport(currentFile, newComponentName, newPsiFile, editor)
  ...
  fun replaceWithNewTag(replaceName: String): PsiElement {
    ...
    val dummyFile = PsiFileFactory.getInstance(project).createFileFromText("dummy.mpx",
                                                                           MpxFileType.INSTANCE, replaceText)
  ...
  private fun optimizeUnusedComponentsAndImports(file: PsiFile) {
    ...
    if (components != null && components.isNotEmpty()) {
      ...
      (file as XmlFile).accept(object : MpxFileVisitor() {
