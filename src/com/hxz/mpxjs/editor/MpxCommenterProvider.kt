...
class MpxCommenterProvider : MultipleLangCommentProvider {
  ...
  override fun getLineCommenter(file: PsiFile, editor: Editor, lineStartLanguage: Language?, lineEndLanguage: Language?): Commenter? {
    ...
    return minimalElement
             ...
             ?.takeIf { it != MpxLanguage.INSTANCE }
             ?.let { elementLanguage ->
               ...
               if (elementLanguage.isKindOf(JavascriptLanguage.INSTANCE)
                   && lineStartLanguage == MpxLanguage.INSTANCE
                 ...
                 xmlParent?.language.takeIf { it !=  MpxLanguage.INSTANCE}
               }
               else
                 elementLanguage
             }
             ?.let { LanguageCommenters.INSTANCE.forLanguage(it) }
           ?: if (lineStartLanguage == MpxLanguage.INSTANCE) {
  ...
  override fun canProcess(file: PsiFile?, viewProvider: FileViewProvider?): Boolean = file?.language == MpxLanguage.INSTANCE

  private fun findMinimalElementContainingRange(file: PsiFile, startOffset: Int, endOffset: Int): PsiElement? {
    val element1 = file.viewProvider.findElementAt(startOffset, MpxLanguage.INSTANCE)
    val element2 = file.viewProvider.findElementAt(endOffset - 1, MpxLanguage.INSTANCE)
