...
private val myBracesCompleter: JSInjectionBracesUtil.InterpolationBracesCompleter =
  MpxInterpolationBracesCompleter(MpxInjector.BRACES_FACTORY)

class MpxJSBracesInterpolationTypedHandler : TypedHandlerDelegate() {
  override fun beforeCharTyped(c: Char, project: Project, editor: Editor, file: PsiFile, fileType: FileType): Result {
    if (fileType != MpxFileType.INSTANCE
        && fileType != HtmlFileType.INSTANCE
        || !isMpxContext(file)) return Result.CONTINUE
...
class MpxInterpolationBracesCompleter(factory: NullableFunction<PsiElement, Pair<String, String>>) :
