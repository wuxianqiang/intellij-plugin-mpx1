...
class MpxBaseLiveTemplateContextType : TemplateContextType("Mpx", MpxBundle.message("mpx.documentation.mpx"),
                                                           EverywhereContextType::class.java) {
  override fun isInContext(file: PsiFile, offset: Int): Boolean {
    return MpxFileType.INSTANCE == file.fileType
  }

  companion object {
    fun evaluateContext(file: PsiFile, offset: Int,
                        scriptContextEvaluator: ((PsiElement) -> Boolean)? = null,
                        notMpxFileType: ((PsiElement) -> Boolean)? = null,
      ...
      if (MpxFileType.INSTANCE != file.fileType) {
        return isMpxContext(file) && notMpxFileType != null && notMpxFileType.invoke(element)
      }
