...
import com.hxz.mpxjs.refactoring.MpxRefactoringUtils

class MpxJSReferenceSearcher : QueryExecutorBase<PsiReference, ReferencesSearch.SearchParameters>(true) {

  override fun processQuery(queryParameters: ReferencesSearch.SearchParameters, consumer: Processor<in PsiReference>) {
    val element = queryParameters.elementToSearch
    val component = MpxRefactoringUtils.getComponent(element)

    if (component != null) {
      // TODO migrate to use MpxModelManager.findEnclosingComponent()
    ...
    if (element is JSQualifiedNamedElement
      ...
      if (name != null && isMpxContext(element)) {
        JSUtils.getMemberContainingClass(element)
          ?.let { MpxModelManager.getComponent(it) }
          ?.castSafelyTo<MpxRegularComponent>()
          ?.template
          ?.source
          ?.let {
            val searchScope = LocalSearchScope(arrayOf(it), MpxBundle.message("mpx.search.scope.template.name"), false)
