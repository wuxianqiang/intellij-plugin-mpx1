...
class MpxJSTypeEvaluator(context: JSEvaluateContext)
  ...
  private fun evaluateTypeFromVForVariable(jsVariable: JSFieldVariable): Boolean {
    if (jsVariable !is MpxJSVForVariable) return false
    val vForExpression = PsiTreeUtil.getParentOfType(jsVariable, MpxJSVForExpression::class.java) ?: return false
  ...
  companion object {
    ...
    private fun useTypeScriptKeyofType(collectionType: JSType): Boolean {
      return (collectionType.isTypeScript || collectionType.sourceElement?.language == MpxJSLanguage.INSTANCE)
