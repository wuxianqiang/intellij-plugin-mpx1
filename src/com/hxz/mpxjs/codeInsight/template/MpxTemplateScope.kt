...
abstract class MpxTemplateScope
/**
 * A scope can be created with parent scope, which contents will be included in the resolution.
 * See [MpxTemplateScope.processAllScopesInHierarchy]
 */
protected constructor(val parent: MpxTemplateScope?) {
  protected val children = ArrayList<MpxTemplateScope>()
  ...
  private fun add(scope: MpxTemplateScope) {
  ...
  fun processAllScopesInHierarchy(processor: Processor<in ResolveResult>): Boolean {
    var scope: MpxTemplateScope? = this
    while (scope != null) {
