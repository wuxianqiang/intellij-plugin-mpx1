...
import com.hxz.mpxjs.index.MPX_CLASS_COMPONENT_MODULE
import com.hxz.mpxjs.index.MPX_MODULE
import com.hxz.mpxjs.index.findScriptTag
import com.hxz.mpxjs.index.hasMpxClassComponentLibrary
...
class MpxInsertHandler : XmlTagInsertHandler() {
  companion object {
    val INSTANCE: MpxInsertHandler = MpxInsertHandler()
  ...
  class InsertHandlerWorker {
    ...
    fun insertComponentImport(context: PsiFile,
      ...
      var obj = MpxComponents.getComponentDescriptor(defaultExportElement)?.initializer

      if (obj !is JSObjectLiteralExpression) {
        if (defaultExportElement !is JSClass) return
        val decorator = MpxComponents.getComponentDecorator(defaultExportElement) ?: return
        val newClass = JSPsiElementFactory.createJSClass("@Component({}) class A {}", decorator)
        val newDecorator = MpxComponents.getComponentDecorator(newClass)!!
        val replacedDecorator = decorator.replace(newDecorator)
        forReformat(replacedDecorator)
        obj = MpxComponents.getComponentDescriptor(defaultExportElement)?.initializer
      }
    ...
    private fun addClassComponentImports(content: JSEmbeddedContent) {
      insertImportIfNotThere("Mpx", true, MPX_MODULE, content)
      insertImportIfNotThere("Component", false, MPX_CLASS_COMPONENT_MODULE, content)
    }
    ...
    private fun createDummyScript(project: Project,
      ...
      val exportText = if (isClass && hasMpxClassComponentLibrary(project)) {
        "@Component\nexport default class $fileName extends Mpx"
      }
      else {
        "export default"
      }
      val text = "<script$langText>$exportText {}\n</script>"
      val dummyFile = PsiFileFactory.getInstance(project).createFileFromText("dummy.mpx", MpxFileType.INSTANCE, text)
