...
class MpxAttributeNameParser private constructor() {
  companion object {
    fun parse(attributeName: CharSequence, context: String? = null, isTopLevel: Boolean = false): MpxAttributeInfo {
      return parse(attributeName) { it.isValidIn(context, isTopLevel) }
    }

    fun parse(attributeName: CharSequence, context: XmlTag): MpxAttributeInfo {
      return parse(attributeName) { it.isValidIn(context) }
    }

    private fun parse(attributeName: CharSequence, isValid: (MpxAttributeKind) -> Boolean): MpxAttributeInfo {
      if (attributeName.length == 0) {
        return MpxAttributeInfo("", MpxAttributeKind.PLAIN)
      }

      val name: String
      val kind: MpxDirectiveKind
      var paramsPos: Int
      var isShorthand = false
      if (attributeName.startsWith(ATTR_EVENT_SHORTHAND)) {
        name = "on"
        kind = MpxDirectiveKind.ON
        paramsPos = 0
        isShorthand = true
      }
      else if (attributeName.startsWith(ATTR_ARGUMENT_PREFIX)) {
        name = "bind"
        kind = MpxDirectiveKind.BIND
        paramsPos = 0
        isShorthand = true
      }
      else if (attributeName.startsWith(ATTR_SLOT_SHORTHAND)) {
        name = "slot"
        kind = MpxDirectiveKind.SLOT
        paramsPos = 0
        isShorthand = true
      }
      //else if (attributeName.startsWith(ATTR_STYLE_NAME)) {
      //  name = "on"
      //  kind = MpxDirectiveKind.ON
      //  paramsPos = 0
      //  isShorthand = true
      //}
        ...
        kind = directiveKindMap[name] ?: MpxDirectiveKind.CUSTOM
        paramsPos = nameEnd
      }
      else {
        ...
        val attributeKind = attributeKindMap.get(name).find(isValid) ?: MpxAttributeKind.PLAIN
        return MpxAttributeInfo(name, attributeKind, parseModifiers(attributeName, nameEnd))
      }
      if (paramsPos >= attributeName.length
        ...
        return MpxDirectiveInfo(name, kind, isShorthand = isShorthand)
      }

      val arguments: String?
      if (attributeName[paramsPos] == ATTR_MODIFIER_PREFIX) {
        return MpxDirectiveInfo(name, kind, null, isShorthand, parseModifiers(attributeName, paramsPos))
      }
      else {
        paramsPos++
      }
      ...
      return MpxDirectiveInfo(name, kind, arguments, isShorthand, parseModifiers(attributeName, paramsPos))
    }
    ...
    private val attributeKindMap: MultiMap<String, MpxAttributeKind> = MultiMap()

    private val directiveKindMap = StreamEx.of(*MpxDirectiveKind.values())
      .mapToEntry({ it.directiveName }, { it })
      .nonNullKeys()
      .toMap()

    init {
      MpxAttributeKind.values().asSequence()
  ...
  open class MpxAttributeInfo internal constructor(val name: String,
                                                   val kind: MpxAttributeKind,
  ...
  class MpxDirectiveInfo internal constructor(name: String,
                                              val directiveKind: MpxDirectiveKind,
                                              val arguments: String? = null,
                                              val isShorthand: Boolean = false,
                                              modifiers: Set<String> = emptySet()) : MpxAttributeInfo(name,
  ...
  @Suppress("unused")
  enum class MpxAttributeKind(val attributeName: String?,
    ...
    JSON_LANG(JSON_ATTRIBUTE_NAME, requiresTag = SCRIPT_TAG_NAME),
    ;
  ...
  enum class MpxDirectiveKind(private val hasName: Boolean = true,
                              val injectJS: Boolean = true,
                              val requiresValue: Boolean = true) {
    CUSTOM(injectJS = false, requiresValue = true),
    BIND,
    ON,
    CLOAK(injectJS = false, requiresValue = false),
    ELSE(injectJS = false, requiresValue = false),
    ELSE_IF(injectJS = false, requiresValue = true),
    FOR(injectJS = false, requiresValue = true),
    HTML(injectJS = false, requiresValue = true),
    IF(injectJS = false, requiresValue = true),
    MODEL(injectJS = false, requiresValue = true),
    ONCE(injectJS = false, requiresValue = false),
    PRE(injectJS = false, requiresValue = false),
    SHOW(injectJS = false, requiresValue = true),
    SLOT(requiresValue = false),
    TEXT(injectJS = false, requiresValue = true),
    STYLE(injectJS = false, requiresValue = true);
