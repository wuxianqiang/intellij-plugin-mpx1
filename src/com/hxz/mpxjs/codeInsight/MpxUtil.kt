...
const val SETUP_ATTRIBUTE_NAME = "name"
...
const val JSON_ATTRIBUTE_NAME = "type"

val MPX_NOTIFICATIONS: NotificationGroup
...
fun detectMpxScriptLanguage(file: PsiFile): String? {
...
fun <T : JSExpression> findExpressionInAttributeValue(attribute: XmlAttribute,
  ...
  val root = when {
    attribute.language === MpxLanguage.INSTANCE ->
      value.children
  ...
  return tryCast((root?.firstChild as? MpxJSEmbeddedExpression)?.firstChild, expressionClass)
}
...
fun <T : PsiElement> resolveSymbolFromNodeModule(scope: PsiElement?, moduleName: String, symbolName: String, symbolClass: Class<T>): T? {
  ...
  return getCachedValue(file, key) {
    //print("$moduleName/$symbolName/${symbolClass.simpleName}")
...
fun resolveLocalComponent(context: MpxEntitiesContainer, tagName: String, containingFile: PsiFile): List<MpxComponent> {
  val result = mutableListOf<MpxComponent>()
  val normalizedTagName = fromAsset(tagName)
  context.acceptEntities(object : MpxModelProximityVisitor() {
