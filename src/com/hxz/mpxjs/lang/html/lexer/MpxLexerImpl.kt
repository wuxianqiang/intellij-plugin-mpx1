...
class MpxLexerImpl(override val languageLevel: JSLanguageLevel,
                   override val project: Project,
                   override val interpolationConfig: Pair<String, String>?)
  : HtmlLexer(MpxMergingLexer(FlexAdapter(_MpxLexer(interpolationConfig))), true), MpxLexer {
  ...
  open class MpxMergingLexer(original: FlexAdapter) : MergingLexerAdapterBase(original) {
    ...
    companion object {
      ...
      fun isLexerWithinUnterminatedInterpolation(state: Int): Boolean {
        return getBaseLexerState(state) == _MpxLexer.UNTERMINATED_INTERPOLATION
      }
