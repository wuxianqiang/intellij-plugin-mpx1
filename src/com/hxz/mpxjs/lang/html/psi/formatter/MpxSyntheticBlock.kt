...
class MpxSyntheticBlock(subBlocks: List<Block>,
                        parent: Block,
                        indent: Indent?,
  ...
  override fun getSpacing(child1: Block?, child2: Block): Spacing? =
    if (isMpxInterpolationBorder(child1, child2) || isMpxInterpolationBorder(child2, child1)) {
      ...
      myXmlFormattingPolicy.settings.getCustomSettings(MpxCodeStyleSettings::class.java).let {
  ...
  companion object {
    fun isMpxInterpolationBorder(child1: Block?, child2: Block?): Boolean =
      (child1 is MpxHtmlBlock || child1 is MpxBlock)
      && (child2 as? AnotherLanguageBlockWrapper)?.node?.psi?.language == MpxJSLanguage.INSTANCE
  }
}
