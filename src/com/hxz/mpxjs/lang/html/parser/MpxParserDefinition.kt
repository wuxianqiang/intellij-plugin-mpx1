...
class MpxParserDefinition : HTMLParserDefinition() {
  ...
  companion object {
    fun createLexer(project: Project, interpolationConfig: Pair<String, String>?): Lexer {
      ...
      return MpxLexerImpl(if (level.isES6Compatible) level else JSLanguageLevel.ES6, project, interpolationConfig)
    }
  }
  ...
  override fun getFileNodeType(): IFileElementType {
    return MpxFileElementType.INSTANCE
  }

  override fun createFile(viewProvider: @NotNull FileViewProvider): @NotNull PsiFile {
    return HtmlFileImpl(viewProvider, MpxFileElementType.INSTANCE)
  }

  override fun createElement(node: ASTNode?): PsiElement {
    if (node?.elementType === MpxElementTypes.MPX_EMBEDDED_CONTENT) {
      return HtmlEmbeddedContentImpl(node)
    }
    return super.createElement(node)
  }

  override fun createParser(project: Project?): HTMLParser = MpxParser()
}
