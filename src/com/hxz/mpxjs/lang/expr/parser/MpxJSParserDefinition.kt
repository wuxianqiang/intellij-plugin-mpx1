...
class MpxJSParserDefinition : JavascriptParserDefinition() {
  companion object {
    private val FILE: IFileElementType = JSFileElementType.create(MpxJSLanguage.INSTANCE)
    private val LOG = Logger.getInstance(MpxJSParserDefinition::class.java)
        ...
        when (FileUtilRt.getExtension(containingFile.name)) {
          EXPRESSION -> {
            val info = containingFile.name
              ...
              .let {
                MpxAttributeNameParser.parse(it)
              }
            MpxJSParser.parseEmbeddedExpression(builder, root, info)
          }
          "js" -> //special case for creation of AST from text
            MpxJSParser.parseJS(builder, root)
          else -> MpxJSParser.parseInterpolation(builder, root)
        }
      }
      else {
        LOG.error("No containing file while parsing Mpx expression.")
      }
