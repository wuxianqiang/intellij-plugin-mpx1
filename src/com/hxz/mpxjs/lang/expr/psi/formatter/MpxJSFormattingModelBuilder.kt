...
class MpxJSFormattingModelBuilder : JavascriptFormattingModelBuilder() {
  override fun createModel(formattingContext: FormattingContext): FormattingModel {
    ...
    if (host is XmlText || element.parent is XmlTag) {
      ...
      val wrapType = WrappingUtil.getWrapType(settings.getCustomSettings(MpxCodeStyleSettings::class.java).INTERPOLATION_WRAP)
      ...
      rootBlock = CompositeJSBlock(listOf(rootBlock), SpacingStrategy { _, _ -> null }, null,
                                   JSFileElementType.getByLanguage(MpxJSLanguage.INSTANCE), jsBlockContext)
    }
    else {
