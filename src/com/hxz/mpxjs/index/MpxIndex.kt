...
import com.hxz.mpxjs.index.MpxIndexBase.Companion.createJSKey

const val MPX_MODULE: String = "@mpxjs/core"
const val MPXTIFY_MODULE: String = "vuetify"
const val BOOTSTRAP_MPX_MODULE: String = "bootstrap-vue"
const val SHARDS_MPX_MODULE: String = "shards-vue"
const val MPX_CLASS_COMPONENT_MODULE: String = "vue-class-component"
...
fun hasMpxClassComponentLibrary(project: Project): Boolean {
  ...
  return CachedValuesManager.getManager(project).getCachedValue(project) {
    ...
    var recordedDependency = packageJsonFiles.any { PackageJsonData.getOrCreate(it).isDependencyOfAnyType(MPX_CLASS_COMPONENT_MODULE) }
    if (!recordedDependency) {
      val psiManager = PsiManager.getInstance(project)
      recordedDependency = packageJsonFiles.any {
        val psiFile = psiManager.findFile(it) ?: return@any false
        NodePackageUtil.hasAnyOfPluginsInstalled(psiFile, listOf(MPX_CLASS_COMPONENT_MODULE))
      }
    }
...
fun getMpxIndexData(element: JSImplicitElement): MpxIndexData? {
  ...
  return MpxIndexData(originalName, nameRef, descriptor, isIndexed, isGlobal)
}

class MpxIndexData(val originalName: String,
