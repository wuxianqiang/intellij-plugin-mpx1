...
import com.hxz.mpxjs.model.source.MpxSourceEntityDescriptor

class TemplateLoaderComponentInfoProvider : MpxContainerInfoProvider {
  ...
  private class TemplateLoaderClassInfo(private val clazz: JSClass) : MpxContainerInfo {
    override val template: MpxTemplate<*>?
      get() = CachedValuesManager.getCachedValue(clazz) {
        clazz.attributeList
          ...
          ?.let { CachedValueProvider.Result.create(MpxFileTemplate(it), clazz, it) }
        ?: CachedValueProvider.Result.create(null as MpxFileTemplate?, clazz, VFS_STRUCTURE_MODIFICATIONS)
      }
  ...
  private class TemplateLoaderInitializerInfo(private val initializer: JSObjectLiteralExpression) : MpxContainerInfo {
    override val template: MpxTemplate<*>?
      get() = CachedValuesManager.getCachedValue(initializer) {
        initializer.context
          ...
          ?.let { resolveToFile(it) }
          ?.let { CachedValueProvider.Result.create(MpxFileTemplate(it), initializer, it) }
        ?: CachedValueProvider.Result.create(null as MpxFileTemplate?, initializer, VFS_STRUCTURE_MODIFICATIONS)
      }
