...
class MpxEslintRuleMappersFactory : EslintRuleMappersFactory {

  override fun createMappers(): List<EslintRuleMapper> = listOf(
    MpxHtmlClosingBracketNewline(),
    MpxHtmlIndent(),
    MpxHtmlQuotes(),
    MpxMustacheInterpolationSpacing(),
    MpxNoSpacesAroundEqualSignsInAttribute(),
    MpxScriptIndent()
  )

  private class MpxHtmlClosingBracketNewline : EslintRuleMapper("mpx/html-closing-bracket-newline") {
  ...
  private class MpxHtmlIndent : EslintRuleMapper("mpx/html-indent") {
    override fun create(values: MutableList<JsonValue>?, eslintConfig: EslintConfig): EslintSettingsConverter {
      ...
      val getIndentOptions = { settings: CodeStyleSettings ->
        if (settings.getLanguageIndentOptions(MpxLanguage.INSTANCE).isOverrideLanguageOptions) {
  ...
  private class MpxHtmlQuotes : EslintRuleMapper("mpx/html-quotes") {
    override fun create(values: MutableList<JsonValue>?, eslintConfig: EslintConfig): EslintSettingsConverter {
  ...
  private class MpxMustacheInterpolationSpacing : EslintRuleMapper("mpx/mustache-interpolation-spacing") {
    override fun create(values: MutableList<JsonValue>?, eslintConfig: EslintConfig): EslintSettingsConverter {
      val spaces = getAlwaysNeverOption(values, true) ?: return EslintSettingsConverter.MISCONFIGURATION
      return object : EslintSettingsConverter {
        override fun inSync(settings: CodeStyleSettings): Boolean =
          settings.getCustomSettings(MpxCodeStyleSettings::class.java)
  ...
  private class MpxNoSpacesAroundEqualSignsInAttribute : EslintRuleMapper("mpx/no-spaces-around-equal-signs-in-attribute") {
    override fun create(values: MutableList<JsonValue>?, eslintConfig: EslintConfig): EslintSettingsConverter {
      return EslintHtmlSettingsConverter(
  ...
  private class MpxScriptIndent : EslintRuleMapper("mpx/script-indent") {
    override fun create(values: MutableList<JsonValue>?, eslintConfig: EslintConfig): EslintSettingsConverter {
      val isTab = values?.getOrNull(0)?.castSafelyTo<JsonStringLiteral>()?.value == "tab"
      val base = if (isTab) 1 else values?.getOrNull(0)?.castSafelyTo<JsonNumberLiteral>()?.value?.toInt() ?: 2
      ...
      return object : EslintSettingsConverter {
        ...
        override fun apply(settings: CodeStyleSettings) {
          settings.getCustomSettings(MpxCodeStyleSettings::class.java).let { custom ->
